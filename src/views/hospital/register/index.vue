...
let data = ref<HospitalDetailInterface.DepartmentInfo>()

onMounted(() => {
    HospitalDetail.DEPARTMENT(($route.query.hoscode) as string).then((ret) => {
        if (ret) {
            data.value = ret
        }
    })
...
<template>
    <div class="main">
        <div class="top">
            <div class="hospital">{{ thedata?.hospital.hosname }}</div>
            <svg t="1691589976225" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
                p-id="7026" width="16" height="16">
                <path
                ...
                <path
                    ...
                    fill="#7f7f7f" p-id="7030"></path>
            </svg>
            <div>{{ thedata?.hospital.param.hostypeString }}</div>
        </div>
        <div class="body">
            <img class="img" :src="`data:image/jpeg;base64,${thedata?.hospital.logoData}`">
            <div class="right">
                <div class="text">挂号规则：</div>
                <div>预约周期：{{ thedata?.bookingRule.cycle }} 放号时间：{{ thedata?.bookingRule.releaseTime }} 停诊时间：{{
                    thedata?.bookingRule.stopTime }}</div>
                <div>具体地址：{{ thedata?.hospital.address }}</div>
                <div>规划路线：{{ thedata?.hospital.route }}</div>
                <div>退号时间：就诊前一工作日{{ thedata?.bookingRule.quitTime }}前取消</div>
                <div class="text">医院预约规则：</div>
                <div v-for=" i in thedata?.hospital.bookingRule">{{ i }}</div>
            </div>
        </div>
        <div class="department">
            <div class="big">
                <div v-for="( i, index) in data" :key="index">{{ i.depname }}</div>
            </div>
            <div class="small">
                <div v-for="( i, index) in data" :key="index">
                    <div class="title">{{ i.depname }}</div>
                    <div v-for="(o, index2) in i.children" :key="index2">{{ o.depname }}</div>
                </div>
